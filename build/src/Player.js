...
Vex.UI.Player.prototype.play = function (self) {
	...
	if (self.currentTime <= event.queuedTime) {
		...
		var timeUntilNextEvent = self.events[self.currentEventIndex + 1].queuedTime - self.events[self.currentEventIndex].queuedTime;
...
Vex.UI.Player.prototype.stop = function () {
	if (this.scheduledId) {
		...
		while (this.events.length) {
			...
			if (event.subtype == "noteOff" || event.subtype == "chordOff") this.fireEvent(event);
		}
	}
};
